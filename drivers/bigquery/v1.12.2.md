...
(driver-bigquery-v1.12.2)=
# BigQuery Driver v1.12.2

[{badge-primary}`Driver Version|v1.12.2`](#driver-bigquery-v1.12.2 "Permalink") {badge-secondary}`Release Date|2026-08-25` {badge-success}`Tested With|BigQuery 1.80.0`
...
[^14]: The driver refuses to truncate to microseconds; it will work if the data is only microsecond precision and not nanosecond precision.
[^15]: This is broken on Google's side; it appears BigQuery treats time64[ns] as int64, and throws a type error
[^16]: This is broken on Google's side, as BigQuery doesn't properly parse Parquet types. See https://github.com/googleapis/google-cloud-python/issues/6542.
[^17]: From experimentation, it appears that BigQuery will accept nanosecond timestamps, but will interpret them as microseconds. Do not use!
