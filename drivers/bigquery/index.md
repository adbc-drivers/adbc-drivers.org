...
[{badge-primary}`Driver Version|v1.12.1`](#driver-bigquery-v1.12.1 "Permalink") {badge-secondary}`Release Date|2026-07-10` {badge-success}`Tested With|BigQuery 1.78.0`
...
[^9]: arrow-go does not support casting binary_view to binary
[^10]: BigQuery cannot natively ingest binary_view
[^11]: arrow-go does not support writing binary view to Parquet
...
[^15]: arrow-go does not support casting string_view to string
[^16]: BigQuery cannot natively ingest string_view
[^17]: arrow-go does not support writing string view to Parquet
[^18]: The driver refuses to truncate to microseconds; it will work if the data is only microsecond precision and not nanosecond precision.
[^19]: This is broken on Google's side; it appears BigQuery treats time64[ns] as int64, and throws a type error
