...
(driver-mssql-v1.6.2)=
# Microsoft SQL Server Driver v1.6.2

:::{note}
This project is not associated with Microsoft.
:::

[{badge-primary}`Driver Version|v1.6.2`](#driver-mssql-v1.6.2 "Permalink") {badge-secondary}`Release Date|2026-09-10` {badge-success}`Tested With|Microsoft SQL Server 2022` {badge-success}`Tested With|Microsoft SQL Server 2025` {badge-success}`Tested With|Fabric Data Warehouse azure`
...
dbapi.connect(driver="mssql", db_kwargs={"uri": "mssql://sa:password@localhost:1433"})
```
...
### EntraID (Azure Active Directory) Authentication
...
#### Connecting with a Pre-Acquired Access Token

If your application already manages its own EntraID token acquisition and refresh (for example, it runs inside an orchestration platform that mints tokens on its behalf), you can hand a token directly to the driver instead of having the driver acquire one itself. Set `fedauth=ActiveDirectoryServicePrincipalAccessToken` and supply the token as the password, either as the password portion of the URI (as with standard SQL authentication) or as an explicit `password` query parameter:

```
mssql://:<access_token>@<host>/?fedauth=ActiveDirectoryServicePrincipalAccessToken

mssql://<host>/?fedauth=ActiveDirectoryServicePrincipalAccessToken&password=<access_token>
```

Despite the name, this accepts any valid EntraID access token, not only tokens obtained via a service principal — the driver forwards the token to the server as-is and does not inspect how it was obtained. The token must:

- Be scoped/audienced for the target resource (e.g. `https://database.windows.net/.default` for Azure SQL Database and Microsoft Fabric).
- Be issued for the correct tenant.
- Not be expired; the driver does not refresh it, so you must reconnect with a fresh token once the current one expires.

:::{note}
This authentication mode is implemented by [Microsoft's official Go MSSQL driver](https://github.com/microsoft/go-mssqldb), but as of this writing is not listed in its README alongside the other `fedauth` values — it is only demonstrated in one of its example programs. It is nonetheless a supported, working authentication path.
:::

:::{warning}
Treat the access token as a secret. Avoid logging connection URIs that embed a token, and prefer sourcing it from a secrets manager or environment variable rather than hardcoding it.
:::
...
:::{warning}
Support for OpenTelemetry is experimental and breaking changes may be made.
:::

If `adbc.telemetry.trace_parent` is set, the traceparent will be propagated to the database via `SET_CONTEXT_INFO` as [specified by OpenTelemetry semantic conventions](https://github.com/open-telemetry/semantic-conventions/blob/v1.37.0/docs/database/sql-server.md#context-propagation). This does result in an extra round-trip to the database before each query.
...
- Microsoft SQL Server `16.0.4275.2 Enterprise Evaluation Edition (64-bit)`

- Microsoft SQL Server `17.0.4085.5 Enterprise Evaluation Edition (64-bit)`
...
[^4]: binary_view is not supported
[^5]: Negative scales are not supported
[^6]: string_view is not supported
