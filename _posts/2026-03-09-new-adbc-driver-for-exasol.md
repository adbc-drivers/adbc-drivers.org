...
The ADBC driver for [Exasol](https://www.exasol.com/), version 0.6.3, is now available via [dbc](https://docs.columnar.tech/dbc/). Just `dbc install exasol` to get the latest version.

The driver supports query execution; more features are in the works. Documentation can be found at [docs.adbc-drivers.org](http://docs.adbc-drivers.org/drivers/exasol/). As the version number implies, this is an early version of the driver. The Exasol Labs community is still working on adding more features and tuning performance, so check back for updates.
